...
def shape_element(element):
    ...
    if element.tag == "node" or element.tag == "way" :
        ...
        for key in element.attrib.keys():
          if key == "version" or key == "changeset" or key == "user" or \
          key == "uid" or key == "lat" or key == "lon" or key == "timestamp":
            pass
          else:
        ...
        try:
          ...
          pass
        for tagnode in element.findall('tag'):
          if problemchars.search(tagnode.get('k')) != None:
            continue
          else:
            if tagnode.get('k')[:5] == "addr:":
              if tagnode.get('k').replace('addr:','').find(':') == -1:
                if node.has_key('address') == False:
                  node['address'] = {}
                tagval = tagnode.get('k').replace('addr:','')
                if tagval == "street":
                    cleaned_name = cleaner.update_name(tagnode.get('v'),cleaner.mapping)
                    node['address'][tagval] = cleaned_name
                else:
        ...
        if element.tag == "way":
          for ndway in element.findall('nd'):
            if node.has_key('node_refs') == False:
              node['node_refs'] = []
            node['node_refs'].append(ndway.get('ref'))
        return node
    else:
      return None
